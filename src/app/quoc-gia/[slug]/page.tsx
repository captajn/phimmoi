...
export default function CountryPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: () => api.getMoviesByCountry(slug, page, filters),
    enabled: !!slug
  });

  const movies = data?.data?.items || [];
  const totalPages = data?.data?.params?.pagination?.totalPages || 1;
  const countryName = data?.data?.titlePage || '';
  ...
  const updateFilter = (key: string, value: string) => {
    ...
    router.push(`/quoc-gia/${slug}?${searchParamsObj.toString()}`);
  };
        ...
        <>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
            {movies.map((movie: Movie) => (
              <Link href={`/phim/${movie.slug}`} key={movie._id} className="group">
