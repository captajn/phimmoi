...
export default function Browse() {
  ...
  // Xác định tiêu đề và API endpoint dựa vào filters
  const getBrowseInfo = () => {
  ...
  const { data, isLoading, error } = useQuery({
    queryKey: ['browseMovies', page, filters],
    queryFn: async () => {
      try {
        ...
        if (filters.category && filters.category.trim() !== '') {
          if (!browseInfo.endpoint.includes(`/the-loai/${filters.category}`)) {
            params.append('category', filters.category);
          }
        }
        
        if (filters.country && filters.country.trim() !== '') {
          if (!browseInfo.endpoint.includes(`/quoc-gia/${filters.country}`)) {
            params.append('country', filters.country);
          }
        }
        
        if (filters.year && filters.year.trim() !== '') {
          if (!browseInfo.endpoint.includes(`/nam/${filters.year}`)) {
